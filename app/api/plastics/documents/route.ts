...
export async function GET() {
  ...
  const { data, error } = await supabase
    ...
    .select(
      "id, direction, doc_kind, doc_no, party_doc_no, doc_date, due_date, status, currency, total_minor, parties!plastics_documents_counterparty_org_fk(name)",
    )
...
export async function POST(req: NextRequest) {
  ...
  if (error) return rpcError("POST /api/plastics/documents", error, "Could not issue this document");

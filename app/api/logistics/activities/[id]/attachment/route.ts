...
// Same bucket the shared ledger uses — an activity id is a UUID unique
// across every table, so a shared/logistics/plastics path can never collide.
...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: activity, error: readErr } = await supabase
    .from("logistics_activities").select("id, attachment_path").eq("id", id).maybeSingle();
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: activity, error: readErr } = await supabase
    .from("logistics_activities").select("id").eq("id", id).maybeSingle();
  if (readErr) return apiErr("Could not load this activity", 500);
  if (!activity) return apiErr("Activity not found", 404);
  ...
  const { data, error } = await supabase
    .rpc("set_logistics_activity_attachment", { p_activity_id: id, p_attachment_path: path })
    .single();

  if (error) {
    return rpcError("POST /api/logistics/activities/[id]/attachment", error, "Could not attach that file");
  }

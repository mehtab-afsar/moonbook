...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: payment, error } = await supabase
    ...
    .select(
      "id, direction, paid_on, method, reference_no, currency, amount_minor, parties!logistics_payments_party_org_fk(name, tax_id, tax_id_kind, address)",
    )
    .eq("id", id)
    .maybeSingle();

  if (error) {
    log.error("GET /api/logistics/payments/[id]/pdf: query failed", { err: error.message, id });
  ...
  const [{ data: org }, { data: allocations }] = await Promise.all([
    ...
    supabase
      ...
      .select("amount_minor, logistics_documents(doc_no, doc_date, total_minor)")
      .eq("payment_id", id)
      .order("created_at"),
  ]);
  ...
  const applications = (allocations ?? [])
    .map((a) => {
      const doc = (a as unknown as { logistics_documents: { doc_no: string | null; doc_date: string; total_minor: number } | null }).logistics_documents;
      if (!doc) return null;
  ...
  try {
    ...
    log.error("GET /api/logistics/payments/[id]/pdf: render failed", {

...
export async function GET() {
  ...
  const { data, error } = await supabase
    ...
    .select("id, direction, paid_on, method, reference_no, currency, amount_minor, parties!logistics_payments_party_org_fk(name)")
...
export async function POST(req: NextRequest) {
  ...
  if (error) return rpcError("POST /api/logistics/payments", error, "Could not record this payment");

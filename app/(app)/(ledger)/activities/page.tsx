...
export default async function ActivitiesPage() {
  ...
  const [
    ...
    supabase
      ...
      .select("id, label_singular, direction, pricing_strategy, pricing_config, uses_job_margin, activity_fields!activity_fields_activity_type_id_fkey(key, label, field_type, options, is_required, archived_at, sort_order)")
      .not("org_id", "is", null)
      .is("archived_at", null)
      .order("sort_order"),
  ...
  const rows: LogRow[] = (activities ?? []).map((a) => {
    ...
    return {
      ...
      uses_job_margin: row.activity_types?.uses_job_margin ?? false,

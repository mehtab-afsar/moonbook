...
export default async function ReceiptPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: payment, error } = await supabase
    .from("payments")
    .select("id, direction, paid_on, method, reference_no, currency, amount_minor, parties!payments_party_id_fkey(name, tax_id, tax_id_kind, region_code, email, phone)")
    .eq("id", id)
    .maybeSingle();
  ...
  const { data: allocations } = await supabase
    .from("allocations")
    .select("id, amount_minor, documents!allocations_target_document_id_fkey(id, doc_no, doc_date, total_minor)")
    .eq("payment_id", id)
    .order("created_at");
  ...
  const applications = (allocations ?? []).map((a) => ({
    ...
    doc: (a as unknown as { documents: { id: string; doc_no: string | null; doc_date: string; total_minor: number } | null }).documents,
  }));
  ...
  return (
    <div className="mx-auto max-w-[1200px] space-y-6 p-8">
      <header className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <Link href="/payments" className="text-[13px] text-ink-2 hover:text-ink">← Receipts</Link>
        ...
        <div className="flex items-center gap-3">
          ...
          <a href={`/api/payments/${payment.id}/pdf`} target="_blank" rel="noreferrer" className={buttonSecondaryClass}>
            Print
          </a>
          <a href={`/api/payments/${payment.id}/pdf?download=1`} className={buttonPrimaryClass}>
            Download PDF
          </a>
        </div>
      </header>
                ...
                {applications.map((a) => (
                  <tr key={a.id} className="border-b border-line-soft last:border-b-0">
                    <td className="px-5 py-3 font-mono text-ink">
                      {a.doc ? (
                        <Link href={`/documents/${a.doc.id}`} className="text-brand hover:underline">
                          {a.doc.doc_no ?? "—"}
                        </Link>
                      ) : "—"}
                    </td>

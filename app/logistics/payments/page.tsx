...
import { ReceiptsPanel, type PaymentRow } from "@/features/logistics/components/ReceiptsPanel";
import { VendorPaymentsPanel } from "@/features/logistics/components/VendorPaymentsPanel";
...
export default async function LogisticsPaymentsPage() {
  ...
  const [
    ...
    supabase.from("logistics_payments").select("id, paid_on, method, reference_no, currency, amount_minor, parties!logistics_payments_party_org_fk(name)").eq("direction", "in").order("paid_on", { ascending: false }).limit(200),
    supabase.from("logistics_payments").select("id, paid_on, method, reference_no, currency, amount_minor, parties!logistics_payments_party_org_fk(name)").eq("direction", "out").order("paid_on", { ascending: false }).limit(200),
    supabase.from("logistics_document_balances").select("document_id, counterparty_id, doc_no, doc_date, due_date, currency, balance_due_minor").eq("direction", "receivable").gt("balance_due_minor", 0),
    supabase.from("logistics_document_balances").select("document_id, counterparty_id, doc_no, doc_date, due_date, currency, balance_due_minor").eq("direction", "payable").gt("balance_due_minor", 0),
    supabase.from("logistics_payment_balances").select("payment_id, applied_minor, unapplied_minor"),
    // Services tagged to a vendor — offered as an optional reference note
    // when paying that vendor, same as on the Invoices → Vendor payment tab.
    supabase
  ...
  const defaultCurrency = org?.base_currency ?? "INR";
  ...
  return (
    <div className="mx-auto max-w-[1200px] space-y-10 p-8">
      <ReceiptsPanel
        heading="h1"
        title="Receipts"
        description="Money received. To pay a vendor, use the Vendor payments section below."
      ...
      <VendorPaymentsPanel
        ...
        openBillsByParty={openPayableByParty}

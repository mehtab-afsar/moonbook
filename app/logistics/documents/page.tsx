    ...
    <div className="space-y-6 p-8">
      <header className="flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="flex gap-3">
          <Link href="/logistics/documents/new/bill" className={buttonSecondaryClass}>Record a bill</Link>
      ...
      <div className="overflow-x-auto rounded-[10px] border border-line bg-white">
        <table className="w-full text-left text-[13.5px]">
          ...
          <tbody>
            {rows.length === 0 && (
              <tr><td colSpan={6} className="px-5 py-10 text-center text-ink-3">Nothing issued yet.</td></tr>
            )}
            {rows.map((d) => {
              const party = (d as unknown as { parties: { name: string } | null }).parties;
              const state = settlementLabel(d.balance, d.status);
              const overdue = d.balance !== null && d.balance.balance_due_minor > 0 && d.due_date !== null && d.due_date < today;
              const tone = overdue ? "overdue" : state.tone;
              return (
                <tr key={d.id} className="border-b border-line-soft last:border-b-0 hover:bg-paper">
                  <td className="px-5 py-3">
                    <Link href={`/logistics/documents/${d.id}`} className="font-mono font-medium text-brand hover:underline">
                  ...
                  <td className="px-5 py-3 font-mono">
                    {d.balance === null || d.balance.balance_due_minor <= 0 ? (
                      ...
                      <span className={overdue ? "text-overdue" : "text-ink"}>{formatMoney(d.balance.balance_due_minor, d.currency, locale)}</span>
                    )}
                  </td>
                  <td className="px-5 py-3">
                    <span className={`rounded-full px-2 py-0.5 text-[11.5px] font-medium ${TONE[tone]}`}>{overdue ? "Overdue" : state.label}</span>
                  </td>
                </tr>
              );
            })}

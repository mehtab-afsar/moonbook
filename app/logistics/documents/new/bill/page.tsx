...
export default async function NewLogisticsBillPage() {
  ...
  const [{ data: org }, { data: parties }, { data: activities, error }] = await Promise.all([
    ...
    supabase
      ...
      .select("id, occurred_on, reference, amount_minor, currency, party_id, bill_to_party_id, vendor_ref")
      .eq("status", "completed")
  ...
  for (const a of activities ?? []) {
    ...
    (byParty[billTo] ??= []).push({
      id: a.id, label: `Vendor charge${a.vendor_ref ? ` · ${a.vendor_ref}` : ""}`,

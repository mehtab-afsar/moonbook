...
export function ReceiptsPanel({
  ...
  partyColumn: string;
  emptyLabel: string;
  /** e.g. "/api/plastics/payments" — the row's PDF opens at `${pdfHrefPrefix}/${id}/pdf`, in a new tab. */
              ...
              return (
                <tr key={p.id} className="border-b border-line-soft last:border-b-0">
                  <td className="px-5 py-3 font-mono text-ink-2">
                    <Link href={`/plastics/payments/${p.id}`} className="text-brand hover:underline">{p.paid_on}</Link>
                  </td>

...
const COPY = {
  invoice: {
    who: "Who are you billing?", what: "What are you billing for?",
    docDate: "Invoice date", submit: "Issue invoice", submitting: "Issuing…",
  },
  bill: {
    who: "Who is this owed to?", what: "What are you being billed for?",
    docDate: "Bill date", submit: "Record bill", submitting: "Recording…",
  },
} as const;
...
export function IssueForm({
  ...
  async function submit(e: React.FormEvent) {
    ...
    const res = await fetch("/api/logistics/documents", {
      method: "POST",
    ...
    router.push(`/logistics/documents/${body.data.document_id}`);
    router.refresh();
  }
    ...
    <form onSubmit={submit} className="max-w-[760px] space-y-5">
      ...
      {chosen.length > 0 && (
        <div className="rounded-md border border-line bg-paper p-4">
          ...
          <p className="mt-1.5 text-[12.5px] text-ink-3">Tax is worked out on the server from your tax settings, then added to this.</p>
        </div>
      )}

...
/**
 * One direction's worth of receipts (or vendor payments): a toggle-to-reveal
 * ReceiptForm, a search box, and the resulting table — the same add-button
 * and search pattern used on activities/documents/expenses, applied here
 * too rather than leaving this list the odd one out.
 */
export function ReceiptsPanel({
  ...
  partyColumn: string;
  emptyLabel: string;
  /** e.g. "/api/payments" — the row's PDF opens at `${pdfHrefPrefix}/${id}/pdf`, in a new tab. */
              ...
              return (
                <tr key={p.id} className="border-b border-line-soft last:border-b-0">
                  <td className="px-5 py-3 font-mono text-ink-2">
                    <Link href={`/payments/${p.id}`} className="text-brand hover:underline">{p.paid_on}</Link>
                  </td>

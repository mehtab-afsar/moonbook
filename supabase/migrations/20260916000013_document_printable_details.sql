-- ───────────────────────────────────────────────────────────────────────────
-- 0013 — issue_document: freeze the printable details onto the document
--
-- Supersedes the version in 20260916000009_ledger_rpcs.sql, identical
-- signature, body carried forward whole. One change, in the issued_snapshot:
-- each line now carries `printable_details`, the activity's `details` values
-- resolved against the field definitions marked `show_on_document`.
--
-- WHY this belongs in the snapshot rather than being joined at render time:
-- an invoice is a statement of fact made on a date. If the PDF rebuilt the
-- details by joining live tables, editing a field label — or archiving the
-- field — would silently rewrite the wording of documents already sent to a
-- customer and filed with a tax authority. Freezing label AND value makes an
-- issued document genuinely immutable.
--
-- This is also the one and only place the descriptive body crosses into the
-- document layer, and it crosses as TEXT: rendered, never computed on. The
-- spine-purity guard allows `details` in this file for exactly this reason.
--
-- Rollback: re-apply the definition from 20260916000009_ledger_rpcs.sql.
-- ───────────────────────────────────────────────────────────────────────────

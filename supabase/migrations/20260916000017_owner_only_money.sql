-- ═══════════════════════════════════════════════════════════════════════════
-- 0017 — issuing and taking money are owner-only in the database too
--
-- Base versions carried forward: issue_document from
-- 20260916000013_document_printable_details.sql (NOT from 0009 — see below),
-- and record_payment from 20260916000009_ledger_rpcs.sql.
--
-- lib/auth/verify.ts states the rule: "issuing, cancelling, exporting and
-- closing a period are all owner-only actions." The app behaved as though that
-- were true — /documents/new redirects a non-owner, and the document page
-- hides the payment form from one — but neither the route nor the RPC checked.
--
-- So a member of staff who opened the network tab could POST /api/documents
-- and issue an invoice, or POST /api/payments and record a receipt. Both
-- returned 201. They are not attackers; they are colleagues with a valid
-- session doing the obvious thing after the button they expected was missing.
--
-- HIDING AN ACTION IS NOT ENFORCING IT. A redirect is a courtesy to someone
-- who cannot do a thing; it is not the reason they cannot. cancel_document and
-- issue_credit_note already carried the guard, which is what made the
-- inconsistency visible the moment anything ran as a non-owner — nothing ever
-- had, in fifty-five tests.
--
-- The guard goes in the RPC rather than only in the route because the RPC is
-- granted to `authenticated` and PostgREST exposes it directly. The route is
-- one way in, not the only one.
--
-- record_activity and update_activity are deliberately NOT changed. Recording
-- what the business did, and correcting it before it is billed, is the
-- everyday work of everyone in the office. The line is drawn at money: what is
-- billed, what is received, what is voided.
--
-- A NOTE ON THE BASE VERSION, because the first cut of this migration got it
-- wrong. issue_document has been superseded once already, by 0013, which added
-- the lateral join that freezes each line's printable details into the
-- snapshot. Copying the body forward from 0009 — the oldest definition, and
-- the one an unqualified search finds first — silently reverted that feature
-- while the `comment on function` still described it. Nothing failed: the
-- migration applied, every guard passed, and invoices quietly stopped carrying
-- the fields that make them readable. Found by a test that asserted on the
-- snapshot's contents rather than on its existence.
--
-- Rollback: re-apply issue_document from 0013 and record_payment from 0009.
-- ═══════════════════════════════════════════════════════════════════════════
...
comment on function public.record_payment(text, uuid, bigint, date, text, jsonb, text, text) is
  'Owner-only since 0017. Records money in or out and, optionally, allocates it against documents in the same transaction.';
...
-- ─── Rollback ──────────────────────────────────────────────────────────────
-- Re-apply issue_document from 20260916000013_document_printable_details.sql
-- and record_payment from 20260916000009_ledger_rpcs.sql.
